...
const numberOfMuscles = labelPosteriorMuscles.length;
const copyData = labelPosteriorMuscles.slice();
...
function checkAnswers(){
    ...
    for(a = 0;a<answers.length;a++){
        if(answers[a] == labelPosteriorMuscles[a])
        {  
            score ++;
        }else{
            wrongAnswers.push(labelPosteriorMuscles[a]);
        }
    }
